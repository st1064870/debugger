...
void GlobalDebuggerUI::SetupMenu(UIContext* context)
{
	...
	UIAction::registerAction("Add Binary View Into Debugger...");
	context->globalActions()->bindAction("Add Binary View Into Debugger...", UIAction([=](const UIActionContext& ctxt) {
		if (!ctxt.binaryView)
			return;
		auto controller = DebuggerController::GetController(ctxt.binaryView);
		if (!controller)
